...
const NO_DURATION_FILE = path.join(FIXTURES_DIR, 'no-duration.png'); // valid file ffprobe reads without error, but has no duration
...
describe('VideoProvider', () => {
  ...
  it('detects real scene changes rather than falling back to even sampling', async () => {
    ...
    // Even-sampling for frameCount=2 over ~4.2s would land at ~1.05s/~3.15s, not the fixture's actual cuts
    // (~2.2s/~3.2s, verified against real ffprobe) — a length-only assertion wouldn't have caught the
    // pkt_pts_time/pts_time field-name bug, which silently fell back to even sampling without erroring.
    expect(frames).toHaveLength(2);
  ...
  it('falls back to even sampling by requesting more frames than detected scene changes exist', async () => {
    ...
    // The fixture has ~3 detectable cuts; asking for 10 forces the
    // fillRemainderWithEven top-up path.
    const frames = await provider.extractFrames(SCENE_CHANGES_VIDEO, {
      frameCount: 10,
      samplingStrategy: SamplingStrategy.SCENE_CHANGE_DETECTION,
  ...
  it('falls back to even sampling when the scene-change probe itself fails (e.g. times out)', async () => {
    // Delegates the duration probe to real ffprobe but hangs only on the lavfi scene-change probe, forcing the
    // timeout path and exercising probeSceneChangeTimestamps's fallback without also breaking the shared duration
    // probe (which a fake binary that always failed would).
